...
function fmtDate(d: Date | string | null) {
    if (!d) return "—";
    return new Date(d).toLocaleDateString("es-PR", { weekday: "short", month: "short", day: "numeric", year: "numeric" });
}
...
function getActions(status: InvoiceStatus): StatusAction[] {
    switch (status) {
        case "DRAFT":
            return [
                { label: "Enviar Factura", status: "SENT", icon: Send, variant: "default" },
        ...
        case "OVERDUE":
            return [
                { label: "Marcar Pagada", status: "PAID_ACTION", icon: CheckCircle2, variant: "default" },
                { label: "Cancelar", status: "CANCELLED", icon: XCircle, variant: "destructive" },
            ];
        default:
            return [];
...
export function InvoiceDetail({ initialInvoice }: { initialInvoice: { id: string } }) {
    ...
    const { data: invoice, isLoading } = trpc.invoice.byId.useQuery({ id: initialInvoice.id });
    ...
    const isOverdue = invoice.dueDate && new Date(invoice.dueDate) < new Date() && !["PAID", "CANCELLED"].includes(invoice.status);
    ...
    const handleAction = (action: StatusAction) => {
        if (action.status === "PAID_ACTION") {
            ...
            updateStatus.mutate({ id: invoice.id, status: action.status as "SENT" | "CANCELLED" });
        }
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="font-heading text-3xl font-bold">{invoice.invoiceNumber}</h1>
                                        ...
                                        {addingItem && (
                                            <tr>
                                                ...
                                                <td className="py-2 text-right font-medium text-muted-foreground">
                                                    {fmtCurrency(parseFloat(newItem.quantity || "0") * parseFloat(newItem.unitPrice || "0"))}
                                                </td>
                            ...
                            {editingNotes ? (
                                <div className="space-y-3">
                                    ...
                                    <div className="flex gap-2">
                                        <Button
                                            size="sm"
                                            onClick={() => updateInvoice.mutate({ id: invoice.id, notes })}
                                ...
                                <p className="text-sm text-muted-foreground whitespace-pre-wrap">{invoice.notes || "Sin notas."}</p>
                            )}
                        </CardContent>
                    </Card>
                            ...
                            <CardContent>
                                <div className="flex items-center justify-between">
                                    ...
                                    <Link href={`/jobs/${invoice.job.id}`}>
                                        <Button variant="outline" size="sm">Ver Trabajo</Button>
                                    </Link>
                                </div>
                            </CardContent>
                        </Card>
                                ...
                                <div className="space-y-3">
                                    {invoice.activities.map((activity) => (
                                        <div key={activity.id} className="flex items-start gap-3">
                                            <div className={`mt-1 h-2 w-2 rounded-full flex-shrink-0 ${activity.type === "PAID" ? "bg-green-500" :
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <Link href={`/clients/${invoice.client.id}`}>
                                <Button variant="outline" size="sm" className="w-full mt-2">Ver Cliente</Button>
                            </Link>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            {invoice.sentAt && (
                                <div>
                                    ...
                                    <p className="text-sm font-medium">{fmtDate(invoice.sentAt)}</p>
                                </div>
                            )}
                            {invoice.paidAt && (
                                <div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-xs text-muted-foreground mb-2">
                                Comparte este enlace con tu cliente para ver la factura en línea.
                            </p>
                            <div className="flex gap-2">
                                <input
                                    readOnly
