          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-emerald-200 dark:border-emerald-800/50">
                <th className="text-left py-1.5 px-2 font-medium text-emerald-700 dark:text-emerald-300">
                  Material
                </th>
                <th className="text-right py-1.5 px-2 font-medium text-emerald-700 dark:text-emerald-300">
                  Precio
                </th>
                <th className="text-right py-1.5 px-2 font-medium text-emerald-700 dark:text-emerald-300">
                  Cant.
            ...
            <tfoot>
              <tr className="border-t-2 border-emerald-300 dark:border-emerald-700">
                <td
                  colSpan={3}
                  className="py-2 px-2 font-bold text-sm text-emerald-800 dark:text-emerald-200"
                >
                  Total Materiales
                </td>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isAudio && (
          <div
            className={`flex items-center gap-2 text-xs text-muted-foreground ${isUser ? "justify-end" : ""}`}
          >
...
function formatMarkdown(text: string): string {
  ...
  processedText = processedText.replace(
    ...
    (match, tableBlock: string) => {
      ...
      let tableHtml =
        '<div class="overflow-x-auto my-3"><table class="w-full text-xs border-collapse">';
