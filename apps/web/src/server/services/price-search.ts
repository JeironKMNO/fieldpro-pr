...
const priceCache = new Map<string, { products: SerpApiProduct[]; expiresAt: number }>();
...
async function searchHomeDepotViaSerpApi(query: string): Promise<SerpApiProduct[]> {
    if (!SERPAPI_KEY) {
        console.warn("[PriceSearch] SERPAPI_API_KEY not configured, skipping Home Depot search");
        return [];
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.warn(`[PriceSearch] SerpAPI returned ${response.status}: ${errorText.slice(0, 200)}`);
            return [];
        }
            ...
            for (const product of data.products.slice(0, 3)) {
                if (product.price && product.price > 0) {
                    ...
                    if (product.thumbnails && Array.isArray(product.thumbnails)) {
                        ...
                        thumbnail = thumbs.find((t: string) => t.includes("300")) ??
                    ...
                    products.push({
                        ...
                        price: product.price,
                        link: product.link ?? `https://www.homedepot.com/s/${encodeURIComponent(query)}`,
                        brand: product.brand,
...
const PRICE_DATABASE: Record<string, { price: number; unit: string; store: string }> = {
    // Concrete & Masonry
    "cement bag 94lb": { price: 12.98, unit: "bag", store: "Home Depot PR" },
    "cement bag": { price: 12.98, unit: "bag", store: "Home Depot PR" },
    "concrete block 6 inch": { price: 1.85, unit: "unit", store: "Home Depot PR" },
    "concrete block 8 inch": { price: 2.45, unit: "unit", store: "Home Depot PR" },
    "rebar #3": { price: 4.50, unit: "piece (20ft)", store: "Ferretería local" },
    "rebar #4": { price: 7.98, unit: "piece (20ft)", store: "Ferretería local" },
    "sand cubic yard": { price: 45.00, unit: "cubic yard", store: "Materialista" },
    "gravel cubic yard": { price: 55.00, unit: "cubic yard", store: "Materialista" },
    "ready mix concrete": { price: 165.00, unit: "cubic yard", store: "CEMEX PR" },
    ...
    "toilet standard": { price: 189.00, unit: "unit", store: "Home Depot PR" },
    "bathroom sink": { price: 85.00, unit: "unit", store: "Home Depot PR" },
    "kitchen faucet": { price: 129.00, unit: "unit", store: "Home Depot PR" },

    // Electrical
    "romex 12/2 wire": { price: 78.98, unit: "250ft roll", store: "Home Depot PR" },
    "romex 14/2 wire": { price: 58.98, unit: "250ft roll", store: "Home Depot PR" },
    ...
    "electrical panel 200a": { price: 189.00, unit: "unit", store: "Home Depot PR" },
    "led bulb": { price: 3.98, unit: "unit", store: "Home Depot PR" },

    // Roofing
    "galvalume roofing sheet": { price: 28.00, unit: "sheet (10ft)", store: "Ferretería local" },
    "roofing nail": { price: 45.00, unit: "box (50lb)", store: "Home Depot PR" },
    "roof sealant": { price: 12.98, unit: "tube", store: "Home Depot PR" },
    "roof coating 5gal": { price: 89.00, unit: "bucket", store: "Home Depot PR" },
    "asphalt shingles bundle": { price: 35.98, unit: "bundle", store: "Home Depot PR" },

    // Flooring
    "ceramic tile 12x12": { price: 1.89, unit: "sq ft", store: "Home Depot PR" },
    "ceramic tile 18x18": { price: 2.49, unit: "sq ft", store: "Home Depot PR" },
    "porcelain tile 24x24": { price: 3.49, unit: "sq ft", store: "Home Depot PR" },
    ...
    "vinyl plank flooring": { price: 2.99, unit: "sq ft", store: "Home Depot PR" },

    // Paint
    "interior paint gallon": { price: 32.98, unit: "gallon", store: "Home Depot PR" },
    "exterior paint gallon": { price: 38.98, unit: "gallon", store: "Home Depot PR" },
    ...
    "epoxy paint gallon": { price: 45.98, unit: "gallon", store: "Home Depot PR" },

    // Windows & Doors
    "aluminum window 3x3": { price: 125.00, unit: "unit", store: "Ferretería local" },
    "aluminum window 4x4": { price: 175.00, unit: "unit", store: "Ferretería local" },
    "interior door": { price: 89.00, unit: "unit", store: "Home Depot PR" },
    "exterior door steel": { price: 289.00, unit: "unit", store: "Home Depot PR" },
    "sliding glass door": { price: 499.00, unit: "unit", store: "Home Depot PR" },

    // Drywall & Finishing
    "drywall 4x8 1/2": { price: 14.98, unit: "sheet", store: "Home Depot PR" },
    "drywall 4x8 5/8": { price: 17.98, unit: "sheet", store: "Home Depot PR" },
    "joint compound 5gal": { price: 18.98, unit: "bucket", store: "Home Depot PR" },
    "drywall tape": { price: 6.98, unit: "roll", store: "Home Depot PR" },
    "corner bead 8ft": { price: 3.48, unit: "piece", store: "Home Depot PR" },

    // Labor rates (Puerto Rico typical rates)
    "general labor": { price: 18.00, unit: "hour", store: "PR Market Rate" },
    "skilled labor": { price: 25.00, unit: "hour", store: "PR Market Rate" },
    "electrician": { price: 35.00, unit: "hour", store: "PR Market Rate" },
    "plumber": { price: 35.00, unit: "hour", store: "PR Market Rate" },
    "painter": { price: 22.00, unit: "hour", store: "PR Market Rate" },
    "tile installer": { price: 28.00, unit: "hour", store: "PR Market Rate" },
    "demolition labor": { price: 20.00, unit: "hour", store: "PR Market Rate" },
};

function searchLocalDatabase(searchKey: string): { price: number; unit: string; store: string } | null {
    // Exact match
    if (PRICE_DATABASE[searchKey]) return PRICE_DATABASE[searchKey];

    // Partial match
    const keys = Object.keys(PRICE_DATABASE);
    const partial = keys.find((k) => searchKey.includes(k) || k.includes(searchKey));
    ...
    for (const key of keys) {
        ...
        const score = matchingWords.length / Math.max(searchWords.length, keyWords.length);
...
function estimatePrice(name: string): number {
    ...
    if (lower.includes("cement") || lower.includes("concrete") || lower.includes("cemento")) return 12.00;
    if (lower.includes("block") || lower.includes("bloque")) return 2.00;
    if (lower.includes("rebar") || lower.includes("varilla")) return 6.00;
    if (lower.includes("lumber") || lower.includes("madera") || lower.includes("2x")) return 8.00;
    if (lower.includes("plywood") || lower.includes("sheet")) return 35.00;
    if (lower.includes("pipe") || lower.includes("tubo") || lower.includes("pvc")) return 8.00;
    if (lower.includes("wire") || lower.includes("cable")) return 65.00;
    if (lower.includes("paint") || lower.includes("pintura")) return 35.00;
    if (lower.includes("tile") || lower.includes("loseta") || lower.includes("ceramica")) return 2.50;
    if (lower.includes("door") || lower.includes("puerta")) return 150.00;
    if (lower.includes("window") || lower.includes("ventana")) return 150.00;
    if (lower.includes("nail") || lower.includes("screw") || lower.includes("clavo") || lower.includes("tornillo")) return 12.00;
    if (lower.includes("labor") || lower.includes("mano de obra")) return 22.00;
    if (lower.includes("drywall") || lower.includes("sheetrock")) return 16.00;
    if (lower.includes("grout") || lower.includes("mortar") || lower.includes("mezcla")) return 15.00;
    if (lower.includes("primer") || lower.includes("sellador")) return 25.00;
    return 15.00;
}
        ...
        const batchResults = await Promise.all(
            batch.map(async (material) => {
                ...
                try {
                    ...
                    console.warn(`[PriceSearch] SerpAPI failed for "${material.name}":`, error);
                }
                ...
                if (localResult) {
                    return {
                        ...
                        quantity: material.quantity,
                        subtotal: Math.round(localResult.price * material.quantity * 100) / 100,
