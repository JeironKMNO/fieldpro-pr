...
async function handleGeminiStreaming(
  ...
  const geminiConfig = {
    ...
    // Lower temperature for more precise/consistent price estimates
    temperature: 0.3,
    topP: 0.95,
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isRateLimit && attempt < maxRetries) {
          ...
          send("status", {
            message: `⏳ Esperando ${delay / 1000}s por rate limit...`,
          });
  ...
  send("status", { message: "🤖 Procesando con Gemini 2.5..." });
...
async function handleOpenAIStreaming(
  ...
  const { openai, GPT4O_MODEL, openaiTools } =
    await import("@/server/services/openai");
    ...
    if (useStreaming) {
      ...
      (async () => {
        try {
          ...
          // ── GPT-4o fallback when Gemini fails ──
          if (provider === "gemini" && process.env.OPENAI_API_KEY) {
            try {
              console.log("[AI Chat] Gemini failed — falling back to GPT-4o");
              send("status", {
                message: isRateLimit
                  ? "⏳ Gemini con límite de uso — usando GPT-4o como respaldo..."
                  : "⚠️ Error en Gemini — usando GPT-4o como respaldo...",
              });
              await handleOpenAIStreaming(messages, images, send);
              send("done", {});
              return;
              ...
              console.error(
                "[AI Chat] GPT-4o fallback also failed:",
                fallbackError
              );
            }
          }
